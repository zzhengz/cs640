...
def hub_tests():
    ...
    reqpkt = mk_pkt(macH1, macH4, ipH1,ipH4)
    s.expect(PacketInputEvent("eth0", reqpkt, display=Ethernet), "LRU_switch_test: receive")
    s.expect(PacketOutputEvent("eth1", reqpkt, display=Ethernet), "LRU_switch_test: [h1]")

    reqpkt = mk_pkt(macH2, macH1, ipH2, ipH1)
    s.expect(PacketInputEvent("eth2", reqpkt, display=Ethernet), "LRU_switch_test: receive")
    s.expect(PacketOutputEvent("eth0", reqpkt, display=Ethernet), "LRU_switch_test: [h1 h2]")

    reqpkt = mk_pkt(macH3, macH1, ipH3, ipH1)
    s.expect(PacketInputEvent("eth3", reqpkt, display=Ethernet), "LRU_switch_test: receive")
    s.expect(PacketOutputEvent("eth0", reqpkt, display=Ethernet), "LRU_switch_test: [h1 h3 h2]")

    reqpkt = mk_pkt(macH4, macH1, ipH4, ipH1)
    s.expect(PacketInputEvent("eth4", reqpkt, display=Ethernet), "LRU_switch_test: receive")
    s.expect(PacketOutputEvent("eth0", reqpkt, display=Ethernet), "LRU_switch_test: [h1 h4 h3 h2]")

    reqpkt = mk_pkt(macH5, macH1, ipH5, ipH1)
    s.expect(PacketInputEvent("eth5", reqpkt, display=Ethernet), "LRU_switch_test: receive")
    s.expect(PacketOutputEvent("eth0", reqpkt,"eth1", reqpkt,"eth2", reqpkt,"eth3", reqpkt,"eth4", reqpkt, display=Ethernet), "LRU_switch_test: [h1 h5 h4 h3 h2]")

    reqpkt = mk_pkt(macH6, macH7, ipH6, ipH7)
    s.expect(PacketInputEvent("eth0", reqpkt, display=Ethernet), "LRU_switch_test: receive")
    s.expect(PacketOutputEvent( "eth2", reqpkt, display=Ethernet), "LRU_switch_test: shoud be broadcast [h6 h1 h5 h4 h3]")

    reqpkt = mk_pkt(macH4, macH5, ipH4, ipH5)
    s.expect(PacketInputEvent("eth0", reqpkt, display=Ethernet), "LRU_switch_test: receive")
    s.expect(PacketOutputEvent( "eth3", reqpkt,  display=Ethernet), "LRU_switch_test: shoud be broadcast [h5 h6 h1 h4 h3]")
