...
10.100.0.0 255.255.0.0 172.16.42.2 router-eth2''')
    ...
    reqpkt2 = mk_ping(ethH2, ethR0, ipH2,ipH4, ttl=64)

    relaypkt = mk_ping(ethR1, ethH4, ipH1,ipH4, ttl=64)
    relaypkt2 = mk_ping(ethR1, ethH4, ipH2,ipH4, ttl=64)
    ...
    arpreq2 = create_ip_arp_request(ethR0, ipR0, ipH1)
    arpreq3 = create_ip_arp_request(ethR2, ipR2, ipH2)
    ...
    s.expect(PacketOutputEvent("router-eth1", arpreq, display=Arp),
             "Router should send ARP request for 10.10.50.250 out router-eth1 interface")

    s.expect(PacketInputEvent("router-eth2", reqpkt2, display=IPv4), 
             "IP packet to be forwarded to 10.10.50.250 should arrive on router-eth2")
    ...
    s.expect(PacketOutputEvent("router-eth0", arpreq2, display=Arp),
             "Router should send ARP request for 192.168.1.239 out router-eth0 interface")
    s.expect(PacketOutputEvent("router-eth2", arpreq3, display=Arp),
